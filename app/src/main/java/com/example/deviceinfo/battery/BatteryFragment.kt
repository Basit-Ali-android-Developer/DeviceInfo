...
class BatteryFragment : Fragment() {
    ...
    private val detailHandler = Handler(Looper.getMainLooper())

    private val graphRunnable = object : Runnable {
        override fun run() {
            _binding?.let { updateBatteryGraph() }
            graphHandler.postDelayed(this, 30_000)
        }
    }

    private val detailRunnable = object : Runnable {
        override fun run() {
            _binding?.let { updateBatteryDetails() }
            detailHandler.postDelayed(this, 2_000)
        }
    }
    ...
    private fun updateBatteryGraph() {
        val batteryLevel = getBatteryLevel(requireContext())
